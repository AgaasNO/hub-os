...
| Path | What it is |
|---|---|
| **`ESSENTIALS.md`** | **Start here.** The whole framework on one page: the 8 layers, the rules that matter, first-hub steps. Most people need nothing else to get going. |
| **`FRAMEWORK.md`** | The full specification (reference, not a cover-to-cover read). 8 layers, minimum implementations, reflex card, instantiation playbook, promotion rules. Go here when you want to understand *why*, or when a layer starts to strain. |
| **`SEMANTIC-OPERATIONS.md`** | The full procedure for running the vault as memory rather than a filing cabinet. Not needed on day one. |
| **`skeleton/`** | The copy-able directory structure for a new hub, including `.claude/` with the session-start hook (a hidden folder, so `cp -r` it along with everything else). Used during instantiation, see Section 6 of `FRAMEWORK.md`. |
...
1. **Read `ESSENTIALS.md`, then `FRAMEWORK.md` Section 1 (Preamble) and Section 3 (Architecture).** You need to understand the organism metaphor and the 8 layers before touching anything.
...
4. **Copy the skeleton** to a new hub directory:
   ...
   ```
   **Vault:** on your first hub, move the copied `vault/` to `~/thehub/vault/`; on later hubs, delete it. There is one vault per user (`FRAMEWORK.md` Section 6.3, Step 6).
...
Current version: **v0.4.0** (2026-09-25). See `FRAMEWORK.md` Section 11 for the full evolution log.
