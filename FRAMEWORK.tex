...
\emph{Version 0.3.0 — snapshot extracted from chiefofstaff hub 2026-04-13, reviewed and patched 2026-04-14, Layer 4 rewritten as semantic memory 2026-04-14}
...
The row pair "actor as character" vs "actor as routing target" is the clearest worked example of the split. \texttt{[[Alex]]} as a character — his background, his strengths, the reason he's hard to delegate to — lives once in \texttt{vault/wiki/people/Alex.md}. \texttt{omar} as an ops routing target — which roles dispatch to him, which CONTEXT.md carries his mechanics — lives in \texttt{ops/}. The vault page links to the ops file; the ops file links back to the vault page; nothing is duplicated.
...
\begin{itemize}
  ...
  \item \textbf{Per-hub vault fragmentation.} User creates a separate \texttt{vault/} inside a new hub directory instead of pointing at the shared \texttt{\textasciitilde{}/thehub/vault/}. The graph splits; cross-hub wikilinks become impossible. Fix: there is one vault per user, not one per hub. New hubs point at the existing vault.
...
Day-one Layer 8 is a pair of short bulleted lists in CLAUDE.md. No scripts, no cron, no directory. The two rituals are \emph{declared}, which is enough to make the coordinator perform them every session.
...
\begin{itemize}
  \item \textbf{Session start:} read OVERVIEW.md, read \texttt{\textasciitilde{}/Thehub/vault/wiki/hot.md}, check in with Gabriel on what's changed, offer COO check-in if ops agents have been used since last session
...
\begin{itemize}
  ...
  \item \textbf{Drift between declared and actual.} CLAUDE.md says "reconcile pending.yaml at session end" but coordinator skips it. Fix: rule is non-negotiable or isn't a rule; if skipping happens, add hook enforcement via Layer 6 or remove the declaration.
...
\subsection{The reflex card (v0.2)}
...
\begin{itemize}
  ...
  \item \textbf{Version the card.} The reflex card has a version number (currently v0.2). Each version change is logged in Section 11.
\end{itemize}
...
\emph{End of FRAMEWORK.md v0.3.0}
