...
Wikilinks give a vault a graph; they don't make it behave like memory. A vault where notes get written and occasionally opened is a \textbf{filing cabinet with cross-references}: it stores meaning but never retrieves it when it would change a decision. Six named operations close that gap, built only from primitives a coordinator already has:

\begin{itemize}
  \item \textbf{Writes:} \emph{primary consolidation} (turn a session's claims into nodes, but only the claims that fail the observer-swap test), \emph{integration} (give every new link its back-reference, in the same commit; \textbf{integration is what upgrades references to edges}), \emph{re-consolidation} (update an old node with a dated overlay and never rewrite the original; \textbf{constitutional interpretation, not editing}).
  \item \textbf{Reads:} \emph{priming} (the session-start load, which checks for a topic pivot), \emph{pattern surfacing} (\textbf{find the question a cluster answers, then check whether that question has a node}), \emph{activation} (load the neighbourhood around a concept when it comes up, and notice when nothing covers it).
\end{itemize}

\textbf{Day one: run primary consolidation, integration and priming.} Add the other three when the vault is big enough that a stale neighbourhood actually misleads a decision. The full procedure (triggers, steps, failure modes, worked examples, and the \texttt{semantic\_neighbors} frontmatter schema) lives in \textbf{[\texttt{SEMANTIC-OPERATIONS.md}](SEMANTIC-OPERATIONS.md)}.
...
The vault is user-scoped (Section 4.4), so it lives at \texttt{\textasciitilde{}/thehub/vault/}, \textbf{not} inside the hub. The skeleton ships a \texttt{vault/} folder only as a template:

\begin{itemize}
  \item \textbf{First hub:} move the copied \texttt{vault/} to \texttt{\textasciitilde{}/thehub/vault/}.
  \item \textbf{Every later hub:} delete the copied \texttt{vault/}. The hub's \texttt{CLAUDE.md} already points at \texttt{\textasciitilde{}/thehub/vault/}.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{4.4 Semantic Memory} — new short subsection \emph{The six semantic operations}, a summary with a day-one subset. The full procedure moved into a new companion file, \textbf{\texttt{SEMANTIC-OPERATIONS.md}}, taken from the version cleaned for strangers in the April handoff kit (in use in the source hub since 2026-04-14). The five phrases marked for survival in that spec are all in the framework text. The companion file includes an honest adoption note: the reads run less often than designed.
  \item \textbf{New entry point: \texttt{ESSENTIALS.md}}, a one-page version of the framework. FRAMEWORK.md is now around 29,000 words, and by its own "rituals too long" anti-pattern nobody reads all of it. The essentials page is what most readers need; FRAMEWORK.md becomes the reference.
  \item \textbf{6.3 Step 6} — resolves a contradiction left over from v0.3.0: the step built the vault inside the hub while 4.4 made it user-scoped. It now says to move the skeleton's \texttt{vault/} to \texttt{\textasciitilde{}/thehub/vault/} on the first hub and delete it on later ones.
